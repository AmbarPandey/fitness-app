...
import { wednesdayExercises } from "../data/exercises";
...
function WednesdayWorkout() {
  ...
  const completedExercises = wednesdayExercises.filter((exercise) => {
    const data = exerciseProgress?.[exercise.name];
    return data?.setsCompleted >= exercise.sets;
  });

  const allCompleted =
    completedExercises.length === wednesdayExercises.length;
  ...
  <h1 className="text-4xl font-bold text-center mb-10">
    Wednesday Workout (Legs + ABS)
  </h1>

      <ProgressBar
        completed={completedExercises.length}
        total={wednesdayExercises.length}
      />
