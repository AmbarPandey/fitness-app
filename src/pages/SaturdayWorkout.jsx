...
import { saturdayExercises } from "../data/exercises";
...
function SaturdayWorkout() {
  ...
  const completedExercises = saturdayExercises.filter((exercise) => {
    const data = exerciseProgress?.[exercise.name];
    return data?.setsCompleted >= exercise.sets;
  });

  const allCompleted =
    completedExercises.length === saturdayExercises.length;
  ...
  <h1 className="text-4xl font-bold text-center mb-10">
    Saturday Workout (Shoulders + Legs)
  </h1>

      <ProgressBar
        completed={completedExercises.length}
        total={saturdayExercises.length}
      />
