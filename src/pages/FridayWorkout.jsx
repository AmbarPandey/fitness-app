...
import { fridayExercises } from "../data/exercises";
...
function FridayWorkout() {
  ...
  const completedExercises = fridayExercises.filter((exercise) => {
    const data = exerciseProgress?.[exercise.name];
    return data?.setsCompleted >= exercise.sets;
  });

  const allCompleted =
    completedExercises.length === fridayExercises.length;
  ...
  <h1 className="text-4xl font-bold text-center mb-10">
    Friday Workout (Biceps + Triceps)
  </h1>

      <ProgressBar
        completed={completedExercises.length}
        total={fridayExercises.length}
      />
